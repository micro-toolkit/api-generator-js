...
function getPayloadWithCurrentUser(payload, currentUserKey, req) {
...
function getResource(metadata, config) {
  ...
  return function(req, res, next){
    ...
    // if current user key is set needs to go in the payload
    var payload = getPayloadWithCurrentUser(
      req.params, metadata.currentUserKey, req);
...
function resourceRelationHandler(relation, config) {
  ...
  return function(req, res, next) {
    ...
    if (currentUserKey) {
      // if current user key is set needs to go in the payload
      payload = getPayloadWithCurrentUser(payload, currentUserKey, req);
    }
