...
function fetchEmbed(relations, config, data){
...
function embeds(metadata, config, query, data) {
  var relations = getEmbedsRelations(query, metadata.relations);
  if (!relations.length || !data) { return Promise.resolve(data); }

  var isArray = _.isArray(data);
  data = isArray ? data : [data];

  return fetchEmbed(relations, config, data)
